...
ssh ws bash -s "$domain" "$history" <<'REMOTE'
set -euo pipefail
domain=$1; history=$2
export PATH="$HOME/.local/bin:$PATH"
echo REDACTED | sudo -S -p '' tailscale serve --bg --https=443 http://127.0.0.1:8790 >/dev/null
echo REDACTED | sudo -S -p '' tailscale serve --bg --https=8443 http://127.0.0.1:8080 >/dev/null
