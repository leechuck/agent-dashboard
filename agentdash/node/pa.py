...
class PersonalAssistant:
    ...
    async def ask(self, p: dict[str, Any]) -> dict[str, Any]:
        ...
        if p.get("topic") == "group":
            system += (
                " Assess research/work progress as of " + datetime.now().isoformat()[:10] + ". "
                "Return ONLY JSON with these fields: state (on_track, watch, attention, unknown), "
                "reason (one-sentence summary, at most 180 characters), progress (0-3 short points "
                "on recent achievements), risks (0-3 short points on current concerns), "
                "next_step (one concrete next milestone or check, with owner if known, at most "
                "200 characters), intervention (specific action Robert must take now, or empty "
                "string; at most 200 characters), evidence (verbatim dated org headings or "
                "message IDs). Each progress/risk point is at most 180 characters. "
                "Copy evidence references exactly from sources without extra labels. "
                "CALIBRATION: on_track means substantive progress with manageable normal work "
                "remaining. watch means a concrete risk or routine follow-up worth monitoring, "
                "but no immediate intervention by Robert is justified. attention is reserved "
                "for a CURRENT material blocker, confirmed missed critical commitment or "
                "decision that requires Robert's intervention now. Name that intervention. "
                "Do not escalate ordinary research uncertainty, negative experiments, planned "
                "future milestones, a missing cc, or incomplete reporting to attention. "
                "An unmentioned deliverable is unverified, NOT proven late or failed. "
                "Approved leave, illness and agreed rescheduling are context, not failures. "
                "Routine mentor-managed risks belong in watch. Read later outcomes before "
